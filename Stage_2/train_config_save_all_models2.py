...
model_efficientnet_b0.classifier = nn.Linear(in_features=model_efficientnet_b0.classifier.in_features, out_features=Global_N_CLASSES)
model_vgg16.head.fc = nn.Linear(in_features=model_vgg16.head.fc.in_features, out_features=Global_N_CLASSES)
model_vgg19.head.fc = nn.Linear(in_features=model_vgg19.head.fc.in_features, out_features=Global_N_CLASSES)

model_efficientnet_b0.load_state_dict(torch.load('../Snake-ID-DeepLearning/models/efficientnet_b0__stage1.pth'))
model_vgg16.load_state_dict(torch.load('../Snake-ID-DeepLearning/models/vgg16__stage1.pth'))
model_vgg19.load_state_dict(torch.load('../Snake-ID-DeepLearning/models/vgg19__stage1.pth'))

model_efficientnet_b0.classifier = nn.Linear(in_features=4096, out_features=Local_N_CLASSES, bias=True)
model_vgg16.head.fc = nn.Linear(in_features=4096, out_features=Local_N_CLASSES, bias=True)
model_vgg19.head.fc = nn.Linear(in_features=4096, out_features=Local_N_CLASSES, bias=True)

for param in model_efficientnet_b0.parameters():
    param.requires_grad = False
model_efficientnet_b0.classifier.requires_grad = True

for param in model_vgg16.parameters():
    param.requires_grad = False
model_vgg16.head.fc.requires_grad = True

for param in model_vgg19.parameters():
    param.requires_grad = False
model_vgg19.head.fc.requires_grad = True
...
def plot_loss_accuracy(train_losses, val_losses, train_accuracies, val_accuracies, train_f1_scores, val_f1_scores, model_name):
    ...
    plt.show()
    plt.savefig(f"{model_name}_moroccan_snakes_V2.png")

# List of models
models = [model_efficientnet_b0, model_vgg16, model_vgg19]
initial_epochs = 10 

# train_loadera and  valid_loader from Data_prepartion2.py
...
for model in models:
    ...
    model_save_path = f"../Snake-ID-DeepLearning/models/{model.__class__.__name__}_stage2.pth"
